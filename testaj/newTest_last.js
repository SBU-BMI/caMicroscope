...
annotools.prototype.drawDots = function() {
    ...
    */
    // end buttons
	removedIdsArr.length = 0 // empty the removedIdsArr
    ...
    svgHtmlDot.on('click', function() {
        ...
        var svgCircle = circleGroup.append('circle')
            ...
            .on('contextmenu', function (d, i) {
				...
				var rmId = 'circle_' + creation;
				removedIdsArr.push(rmId);
				console.log('removedIdsArr in append: ' + removedIdsArr);
                d3.selectAll('g #' + 'circle_' + creation).remove();
            });
		
		console.log('removedIdsArr after append: ' + removedIdsArr);
		...
		*/
		...
		d3.selectAll('.dot').each( function(d, i) {
			...
			//push if not removed
			var isIdRemoved = false;
			for ( var i = 0; i < removedIdsArr.length; i++ ) {
				
				if ( removedIdsArr[i] === tmpId ){
			...
			if (!isIdRemoved) {
			    circleIdsArr.push(tmpId);
			}
			 
			console.log('circleIdsArr in loop: ' + circleIdsArr);
		...
		console.log('circleIdsArr after loop: ' + circleIdsArr);
