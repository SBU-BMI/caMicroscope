        ...
        jQuery.get(url, function(d){
		  ...
		  */
          ...
          jQuery("#deleteAnnot").click(function(e) {
			...
			console.log(data.properties.annotations.username);
			console.log(data.properties.annotations.secret);
			console.log('has own properties: ' + data.properties.annotations.hasOwnProperty('username')); 
			console.log('sessionusername: ' + sessionUsername);
			console.log('annotools.username: ' + annotools.username);
			  
            if(data.provenance.analysis.source == "human") {
				
			    var isUsernameExist = data.properties.annotations.hasOwnProperty('username');
			    if (isUsernameExist) {
			        dataUsername = data.properties.annotations.username[0];
				
				    if (dataUsername == sessionUsername){
					   //alert('Authorized');
					   var payload = {
                        "id": id,
                        "secret": ''
					   ...
					   jQuery.ajax({
			            ...
			            alert('Error deleting markup! You are not authorized to perform that operation');
						jQuery("#panel").hide("slide");
                        self.getMultiAnnot();
			        }
			     }
